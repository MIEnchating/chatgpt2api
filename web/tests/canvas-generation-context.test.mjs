...
test("video node bindings resolve frames, Kling prompts, images, and element resources", () => {
  const nodes = [
    node("target", "video", {
      generation_video_first_frame_node_id: "first",
      generation_video_last_frame_node_id: "last",
      generation_video_kling_image_node_ids: ["character"],
      generation_video_kling_multi_prompt: [{ text_node_id: "shot", duration: "3" }],
      generation_video_kling_element_list: [{ name: "角色", description: "主角", node_ids: ["element-image", "element-video", "element-audio"] }],
    }),
  ...
  assert.deepEqual(buildCanvasGenerationContext("target", nodes, connections, "主提示词"), {
    prompt: "主提示词\n\n电影光影",
    referenceImageURLs: ["/images/character.png", "/images/ordinary.png"],
    firstFrameURL: "/images/first.png",
    lastFrameURL: "/images/last.png",
    referenceVideoURLs: [],
    referenceAudioURLs: [],
    videoMultiPrompt: [{ prompt: "镜头缓慢推进", duration: "3" }],
    videoElementList: [{
      name: "角色",
      description: "主角",
      references: [
        { kind: "image", url: "/images/element.png" },
        { kind: "video", url: "/videos/element.mp4" },
        { kind: "audio", url: "/audios/element.mp3" },
      ],
    }],
    textCount: 2,
    imageCount: 3,
