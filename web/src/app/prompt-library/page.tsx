...
export default function PromptLibraryPage() {
  ...
  const savePrompt = async (prompt: BananaPrompt) => {
    try {
      ...
      try {
        const remote = await fetchMyAssets();
        assets = mergeMyAssets(remote, assets);
      } catch {
        // Keep the local cache usable during a temporary storage outage.
      }
      ...
      try {
        const synced = await syncMyAssets(next);
        saveMyAssets(session.key, synced);
      } catch (error) {
  ...
  return <ImagePromptMarket open presentation="page" initialSource={initialSource} onOpenChange={() => undefined} onApplyPrompt={(prompt) => { stagePromptForWorkbench(prompt); navigate("/image"); }} onSavePrompt={savePrompt} />;
}
