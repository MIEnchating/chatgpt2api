        ...
        <div className="grid gap-5 md:grid-cols-[minmax(260px,1fr)_340px]">
          ...
          <div className="space-y-5 py-1">
            <div className="space-y-2 text-sm">
              ...
              {sourceLong >= CANVAS_MAX_UPSCALE_LONG_EDGE ? <p className="text-xs font-medium text-destructive dark:text-rose-300">图片已达到 4K，无需放大</p> : null}
            </div>
            <div className="space-y-2 text-sm">
...
export function CanvasMaskDialog({ sourceURL, open, busy, model, models, onModelChange, onClose, onConfirm }: {
  ...
  const modelOptions = Array.from(new Set(models.map((value) => value.trim()).filter(Boolean)));
  const modelAvailable = modelOptions.includes(model.trim());
  ...
  async function submit() {
    ...
    if (!modelAvailable) return setError("请选择全局模型设置中已启用的图片模型");
    setSubmitting(true);
    try {
...
function AngleControl({ label, value, min, max, step, suffix, onChange }: { label: string; value: number; min: number; max: number; step: number; suffix: string; onChange: (value: number) => void }) { return <label className="grid grid-cols-[72px_1fr_56px] items-center gap-3 text-sm"><span>{label}</span><Slider min={min} max={max} step={step} value={value} onChange={(event) => onChange(Number(event.target.value))} aria-label={label} /><strong className="text-right">{Number.isInteger(value) ? value : value.toFixed(1)}{suffix}</strong></label>; }
