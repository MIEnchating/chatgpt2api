...
export type CanvasCameraControlOptions = {
  enabled: boolean;
  camera: string;
  lens: string;
  focal_length: number;
...
export const DEFAULT_CAMERA_CONTROL: CanvasCameraControlOptions = {
  enabled: false,
...
function describeFocalLength(mm: number): string {
...
function describeAperture(f: number): string {
...
export function applyCameraPrompt(prompt: string, control?: Partial<CanvasCameraControlOptions>): string {
  if (!control?.enabled) return prompt;
  const cameraControl = { ...DEFAULT_CAMERA_CONTROL, ...control };
  ...
  const cameraPrompt = [
    ...
    lens.profilePrompt,
    describeFocalLength(cameraControl.focal_length),
    describeAperture(cameraControl.aperture),
    'keep the subjects, scene, and action unchanged — apply these as optical and sensor characteristics only',
    '[camera body: ' + camera.shortTag + ' · lens: ' + lens.shortTag + ' · focal: ' + cameraControl.focal_length + 'mm · aperture: f/' + cameraControl.aperture + ']',
  ].join(', ');
