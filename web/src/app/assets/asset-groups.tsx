  ...
  return <>
    <div className="flex min-w-0 shrink-0 flex-wrap items-center gap-2 border-b border-border px-4 py-3 sm:px-5" data-asset-group-toolbar>
      ...
      <Select value={filter} onValueChange={onFilterChange} disabled={disabled}>
        <SelectTrigger className="h-9 w-[180px] max-w-full" aria-label="筛选素材分组"><SelectValue placeholder="素材分组" /></SelectTrigger>
      ...
      <Button variant="ghost" size="icon" className="size-9" disabled={disabled} aria-label="新建分组" title="新建分组" onClick={() => { setName(""); setCreatingKeys([...selectedKeys]); setDialog("create"); }}><FolderPlus className="size-4" /></Button>
      {current ? <><Button variant="ghost" size="icon" className="size-9" disabled={disabled} aria-label="重命名分组" title="重命名分组" onClick={() => { setGroupID(current.id); setName(current.name); setDialog("rename"); }}><Pencil className="size-4" /></Button><Button variant="ghost" size="icon" className="size-9" disabled={disabled} aria-label="删除分组" title="删除分组" onClick={() => { setGroupID(current.id); setDialog("delete"); }}><Trash2 className="size-4" /></Button></> : null}
      {state.loading ? <span className="text-xs text-muted-foreground">正在加载分组...</span> : state.error ? <div role="alert" className="flex min-w-0 items-center gap-2 text-xs text-destructive"><span className="break-words">分组读取失败</span><Button variant="ghost" size="icon" className="size-9" title="重试读取分组" aria-label="重试读取分组" onClick={state.reload}><RefreshCw className="size-4" /></Button></div> : null}
      {selectedKeys.length > 0 ? <div className="ml-auto flex min-w-0 max-w-full flex-wrap items-center gap-2">
        <Select value="" disabled={disabled || !state.groups.length} onValueChange={async (id) => { if (await state.mutate("PATCH", { id, add: selectedKeys })) toast.success(`已添加 ${selectedKeys.length} 个素材到分组`); }}>
          <SelectTrigger className="h-9 w-[160px]" aria-label="将所选素材添加到分组"><SelectValue placeholder="添加到分组" /></SelectTrigger>
          <SelectContent>{state.groups.map((group) => <SelectItem key={group.id} value={group.id}>{group.name}</SelectItem>)}</SelectContent>
        </Select>
        {current ? <Button variant="outline" size="sm" className="h-9" disabled={disabled || !selectedKeys.some((key) => current.assetKeys.includes(key))} onClick={async () => { if (await state.mutate("PATCH", { id: current.id, remove: selectedKeys })) toast.success("已从分组移出所选素材"); }}>移出分组</Button> : null}
      </div> : null}
    </div>
