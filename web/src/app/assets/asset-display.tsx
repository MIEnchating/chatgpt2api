...
export function AssetCard({ asset, selected = false, eager = false, onSelectedChange, onOpen, onEdit, onDelete, onCopy, onDownload }: { asset: MyAsset; selected?: boolean; eager?: boolean; onSelectedChange?: (selected: boolean) => void; onOpen: () => void; onEdit?: () => void; onDelete?: () => void; onCopy: () => void; onDownload: () => void }) {
  ...
  return (
    <article
      ...
      className="interactive-card group relative flex h-full min-w-0 flex-col overflow-hidden rounded-xl border border-border bg-card shadow-[var(--shadow-card)]"
    >
      {onSelectedChange ? (
      ...
      <button type="button" className="interactive-card-trigger block w-full overflow-hidden text-left" onClick={onOpen} aria-label={`查看素材 ${asset.title}`}>
        <div className="flex aspect-[16/10] items-center justify-center overflow-hidden bg-muted/35">
          {previewCoverURL ? (
            ...
            <AssetVideoThumbnail key={mediaURL} src={mediaURL} eager={eager} />
          ) : asset.kind === "audio" ? (
            <div className="flex flex-col items-center gap-2 text-primary">
              <span className="flex size-14 items-center justify-center rounded-full bg-primary/10"><AudioLines className="size-7" /></span>
              <span className="text-xs font-medium">音频素材</span>
            </div>
          ) : (
            <div className="flex w-full flex-col items-center gap-3 px-6 text-center">
              <span className="flex size-12 items-center justify-center rounded-lg bg-primary/10 text-primary"><Icon className="size-6" /></span>
      ...
      <div className="mt-auto flex min-h-12 items-center gap-1 border-t border-border px-3 py-2">
        <Button type="button" variant="ghost" size="sm" className="h-9 px-2 text-xs" onClick={onOpen}><Eye className="size-3.5" />查看</Button>
        {asset.kind === "text" ? <Button type="button" variant="ghost" size="sm" className="h-9 px-2 text-xs" onClick={onCopy}><Copy className="size-3.5" />复制</Button> : <Button type="button" variant="ghost" size="sm" className="h-9 px-2 text-xs" onClick={onDownload}><Download className="size-3.5" />下载</Button>}
        {onEdit ? <Button type="button" variant="ghost" size="sm" className="h-9 px-2 text-xs" onClick={onEdit}><Pencil className="size-3.5" />编辑</Button> : null}
        {onDelete ? <Button type="button" variant="ghost" size="icon" className="ml-auto size-9 text-destructive hover:bg-destructive/10 hover:text-destructive" onClick={onDelete} aria-label="删除素材" title="删除"><Trash2 className="size-3.5" /></Button> : null}
      </div>
    </article>
  );
}
...
export function AssetPreview({ asset, onClose, onCopy, onDownload }: { asset: MyAsset | null; onClose: () => void; onCopy: () => void; onDownload: () => void }) {
  ...
  const copyPrompt = async () => {
    ...
    try {
      ...
      if (!field.isConnected || field !== promptRef.current || field.value !== prompt) return;
      field.focus({ preventScroll: true });
      field.select();
      field.scrollIntoView({ block: "nearest" });
      toast.error(window.isSecureContext ? "浏览器未允许自动复制" : "当前 HTTP 连接不支持自动复制", {
        description: window.isSecureContext ? "提示词已选中，可手动复制或检查浏览器剪贴板权限。" : "提示词已选中，可手动复制；使用 HTTPS 访问可启用自动复制。",
      });
    }
  };
      ...
      <DialogContent scrollable={false} className="flex max-h-[90dvh] w-[min(94vw,760px)] max-w-none flex-col gap-4 overflow-hidden">
        ...
        <ScrollArea data-asset-preview-scroll tabindex={0} ariaLabel="素材详情内容" className="min-h-0 flex-1" viewportClassName="overscroll-contain pr-3">
          <div className="space-y-4">
            ...
            {asset && asset.kind !== "text" && prompt ? <label className="grid gap-2 text-sm font-medium"><span>提示词</span><Textarea key={asset.id} ref={promptRef} aria-label="提示词" value={prompt} readOnly rows={6} className="resize-none font-normal leading-6" /></label> : null}
            {asset ? <div className="grid gap-2 text-sm sm:grid-cols-2"><Info label="媒体信息" value={assetMediaSummary(asset)} />{assetModel(asset) ? <Info label="生成模型" value={assetModel(asset)} /> : null}{asset.tags.length ? <Info label="标签" value={asset.tags.join("、")} className="sm:col-span-2" /> : null}<Info label="来源" value={asset.source || "未标注"} /><Info label="创建时间" value={formatAssetCreatedTime(asset.createdAt)} /><Info label="可见范围" value={asset.visibility === "public" ? "公开" : "个人"} /><Info label="所有者" value={asset.ownerName || (asset.owned !== false ? "我" : "未知")} />{asset.note ? <Info label="备注" value={asset.note} className="sm:col-span-2" /> : null}</div> : null}
          </div>
        </ScrollArea>
        <DialogFooter>{asset?.kind === "text" ? <Button type="button" variant="outline" onClick={onCopy}><Copy />复制文本</Button> : null}{asset && asset.kind !== "text" && prompt ? <Button type="button" variant="outline" onClick={() => void copyPrompt()}><Copy />复制提示词</Button> : null}{asset && asset.kind !== "text" ? <Button type="button" variant="outline" onClick={onDownload}><Download />下载{assetKindLabel(asset.kind)}</Button> : null}<Button type="button" onClick={onClose}>关闭</Button></DialogFooter>
      </DialogContent>
    </Dialog>
  );
}

function AssetVideoThumbnail({ src, eager }: { src: string; eager: boolean }) {
  ...
  return <video ref={videoRef} data-asset-video-thumbnail src={shouldLoad ? `${src}#t=0.1` : undefined} muted playsInline preload={shouldLoad ? "metadata" : "none"} className="size-full object-cover transition-transform duration-300 ease-out group-hover:scale-[1.025]" />;
}

function Info({ label, value, className = "" }: { label: string; value: string; className?: string }) {
  return <div className={`min-w-0 rounded-lg border border-border px-3 py-2.5 ${className}`}><p className="text-[11px] text-muted-foreground">{label}</p><p className="mt-1 break-words text-xs leading-5">{value || "无"}</p></div>;
}
