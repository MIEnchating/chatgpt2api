...
const ACTIVE_STATUSES = new Set<GenerationTaskStatus>(["queued", "running"]);
const TERMINAL_STATUSES = new Set<GenerationTaskStatus>(["success", "error", "cancelled"]);
const MAX_CONCURRENT_CONVERSATION_QUEUE_RUNNERS = 3;

export function deriveGenerationTaskStatus(slots: GenerationTaskSlot[]) {
  const loading = slots.filter((slot) => slot.status === "loading");
  const running = loading.filter((slot) => slot.taskStatus === "running").length;
  const failed = slots.filter((slot) => slot.status === "error");
  const failedCount = failed.length;
  const success = slots.filter((slot) => slot.status === "success").length;
  const cancelled = slots.filter((slot) => slot.status === "cancelled").length;
  const messages = slots.filter((slot) => slot.status === "message").length;
  const outcome = `成功 ${success} 张${failedCount ? `，失败 ${failedCount} 张` : ""}${cancelled ? `，终止 ${cancelled} 张` : ""}`;
  if (running > 0) return { status: "generating" as const, error: undefined };
  if (loading.length > 0) return { status: "queued" as const, error: undefined };
  if (failedCount > 0) return { status: "error" as const, error: failed.find((slot) => slot.error)?.error || outcome };
  if (cancelled > 0) return { status: "cancelled" as const, error: outcome };
  if (success > 0) return { status: "success" as const, error: undefined };
  if (messages > 0) return { status: "message" as const, error: undefined };
  return { status: "queued" as const, error: undefined };
}
...
function statusRank(status: GenerationTaskStatus) {
  if (status === "success") {
    return 3;
  }
...
export function isTaskActive(status: GenerationTaskStatus) {
  return ACTIVE_STATUSES.has(status);
}

function isTaskTerminal(status: GenerationTaskStatus) {
...
/** Explicit per-output state is authoritative when present. */
export function effectiveTaskOutputStatus(
  taskStatus: GenerationTaskStatus,
  outputStatus?: GenerationOutputStatus,
): GenerationOutputStatus {
  if (outputStatus) {
    return outputStatus;
  }
...
function hasTaskOutput(item: CreationTaskData | undefined) {
...
export function effectiveTaskSlotStatus(
  taskStatus: GenerationTaskStatus,
  outputStatus: GenerationOutputStatus | undefined,
  data: CreationTaskData | undefined,
): GenerationOutputStatus {
  const status = effectiveTaskOutputStatus(taskStatus, outputStatus);
...
function mergeTaskOutputStatuses(
...
function mergeStoredImageSnapshot(
...
function mergeImageTurnSnapshot(previous: ImageTurn, incoming: ImageTurn, preferIncoming: boolean): ImageTurn {
  ...
  // regeneration). Only an empty list may be filled from the other snapshot.
