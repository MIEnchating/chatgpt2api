...
本文是分析日期当时的历史记录。“Go 后端实现”“我们当前实现”和 `internal/backend/*` 路径均指当时已移除的直接上游实现，不是当前中转服务架构。本文的浏览器 HAR 与早期单步 Sentinel 脚本是不同观测；复用任一协议结论前必须重新抓包验证。
...
旧本地 Responses 参数校验使用 16 的倍数、最大边长 3840px 和最大像素数 8,294,400 等约束；这不是上游实际输出保证。本节实抓的 1254×1254 已表明实际输出边长可能不是 16 的倍数。
